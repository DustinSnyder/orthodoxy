...
export function Layout() {
  return (
    <div className="mx-auto flex min-h-dvh max-w-3xl flex-col bg-white text-orthodox-ink md:max-w-4xl">
      <header className="sticky top-0 z-20 border-b border-orthodox-gold/55 bg-white/95 px-4 py-3 backdrop-blur md:px-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="font-display text-2xl font-semibold text-orthodox-gold md:text-3xl">
              Orthodox Path
            </p>
            <p className="text-sm text-orthodox-muted">Catechumen Companion</p>
          </div>
          <div
            className="flex h-11 w-11 items-center justify-center rounded-full border border-orthodox-gold/60 text-xl text-orthodox-gold"
            aria-hidden
          >
            ☦
          </div>
        ...
        <nav className="mt-3 hidden border-t border-orthodox-gold/50 pt-3 md:block" aria-label="Primary">
      ...
      {/* Mobile bottom nav */}
      <nav
        className="sticky bottom-0 z-20 border-t border-orthodox-gold/55 bg-white/95 safe-bottom backdrop-blur md:hidden"
        aria-label="Primary"
      >
